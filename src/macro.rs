...
macro_rules! mp4box_gen {
    ...
    {
        @cond
        $input:ident $state:ident;
        [Option<[$($type:tt)*]>],
        $cond:expr,
    } => {
        if $cond {
            Some(mp4box_gen! { @cond $input $state; [$($type)*], true, })
        } else {
            None
        }
    };
    { // Final layer
        ...
        if $cond {
            mp4box_gen! { @read $input $state; $type }
        } else {
    ...
    // Base Type
    {
        ...
        paste::paste! {
            ...
            impl crate::r#macro::Mp4BoxTrait for [<Box $name>] {
                const TYPE: u32 = u32::from_ne_bytes(*bstringify::bstringify!([<$name:lower>]));
            ...
            impl Mp4BoxTrait for [<Box $name>] {
                ...
                fn parse(input: &[u8]) -> Self {
                    ...
                    while !is_empty(input, &state) {
                        data.push(mp4box_gen!{ @read input state; $type });
                    }
...

    {
        ...
        $($sname:ident $(: $stype:ident $(= <$svtype:tt>)?)? $({
            $(
                $field:ident: $ftype:tt $([if $fcond:expr])?
            ),+ $(,)?
        ...
        paste::paste! {
            ...
            pub(crate) fn parse_box(input: &[u8], state: &mut ParserState) -> Option<Mp4Box> {
                ...
                match data.0 {
                    $([<Box $sname>]::TYPE => {
                        Some(Mp4Box::$sname(Box::new([<Box $sname>]::parse(data.1))))
                    })*
